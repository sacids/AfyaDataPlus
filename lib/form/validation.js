...
export function evaluateField1(type, field, formData) {

  const expression = field && field[type];
  if (!expression) return true;

  //console.log('orig expre', expression, type, field.name, JSON.stringify(formData, null, 4));
  try {
    ...
    parsedExpression = parsedExpression
      ...
      .replace(/\b\|\|\b/g, ' or ') // Replace || with or
      .replace(/\b!([^=])/g, 'not $1') // Replace ! with not
    ...
    // Create evaluation context with form data and helper functions
    const context = {
      ...formData,
    ...
    try {
      ...
      // Fallback to mathjs for complex expressions if needed
      console.log('Function evaluation error:', field.name, e, 'Expression:', parsedExpression);
      try {

        const result = math.evaluate(parsedExpression, context);
        return type === 'calculation' ? result : Boolean(result);
      } catch (mathError) {
...
export function evaluateField(type, field, formData) {
  ...
  try {
    ...
    // Step 3: Normalize expression for evaluation - CONVERT TO JAVASCRIPT SYNTAX
    parsedExpression = parsedExpression
      .replace(/(?<![\=\>\<\!])\s*=\s*(?![\=\>\<\!])/g, ' == ') // Replace = with ==
