...
export function evaluateExpression(expression, formData, currentFieldName) {
  ...
  try {
    // Manually run the parsing steps without calling evaluateField
...
export function evaluateField(type, field, formData) {
  ...
  // Track the current field in the path
  const currentPath = `${type}:${field?.name}`;
  evaluationPath.push(currentPath);

  // Check for circular dependency in the path
  const pathString = evaluationPath.join(' → ');
  const fieldCount = evaluationPath.filter(p => p.includes(field?.name)).length;
  ...
  if (fieldCount > 3) {
    console.error('🚨 CIRCULAR DEPENDENCY DETECTED:', {
      ...
      path: pathString,
      message: `${field?.name} appears ${fieldCount} times in evaluation path`
    });
  }

  if (evaluationDepth > MAX_EVAL_DEPTH) {
    console.error('🚨 EVALUATION DEPTH EXCEEDED:', {
      type,
      fieldName: field?.name,
      depth: evaluationDepth
    });
  ...
  try {
    ...
    try {
      ...
      if (type === 'calculation') {
        if (typeof result === 'string') {
          if (result.includes('null') || result.includes('undefined')) {
            console.warn('⚠️ Invalid calculation result:', field.name, result);
            return 0;
          }
        }
    ...
    evaluationDepth--;
    evaluationPath.pop(); // ADD THIS: Remove the field from the path after finishing
    if (evaluationDepth === 0) {
      evaluationPath.length = 0; // Completely clear if we are back at the root
    }
  }
}
