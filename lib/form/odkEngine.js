...
/**
 * Evaluates an ODK calculation expression.
 * Ensures numeric strings are treated as numbers to avoid string concatenation.
 */
export const evaluateCalculation = (expression, formData) => {
  ...
  try {
    ...
    const values = keys.map((key) => {
      ...
      // This ensures that + means addition, not concatenation.
    ...
    console.error("ODK Calculation Error:", { expression, error: e.message });
    return '';
  }
};
