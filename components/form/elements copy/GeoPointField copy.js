...
const GeoPoint = ({ element }) => {
  ...
  const clearGeoPoint = () => {
    Alert.alert('Remove Geopoint', 'Clear this location?', [
      ...
      { text: 'Remove', style: 'destructive', onPress: () => updateField(element.name, null) },
    ]);
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={[styles.mapContainer, fieldError ? styles.inputError : null, { height: 200, overflow: 'hidden', borderRadius: 8 }]}>
        {geoValue ? (
          <>
            <MapView
              style={styles.map}
