...
const FormPage = ({ pageIndex }) => {
  ...
  useEffect(() => {
    if (!page || isCalculating.current) return;
    //console.log('FormPage rendering page:', pageIndex, 'hasVisibleFields:', hasVisibleFields);

    // Use a temporary object to batch updates rather than calling updateFormData in a loop
      ...
      Object.values(fieldGroup).forEach((field) => {
        if (field.type === 'calculate' && field.calculation) {
          try {
            ...
            console.log('Calculating field:', field.name, 'value:', calculatedValue);

            // STRICT CHECK: Only update if the value is truly different
            if (calculatedValue !== formData[field.name]) {
              updatedData[field.name] = calculatedValue;
              hasChanges = true;
            }
          } catch (error) {
            console.error(`Error calculating ${field.name}:`, error);
            console.log('Field data:', JSON.stringify(field));
            console.log('Form data at error:', JSON.stringify(formData));
          }
        }
      });
    });
    ...
    if (hasChanges) {
      console.log('Updating form data with calculated values');
      // Update the store once with the batch of changes
